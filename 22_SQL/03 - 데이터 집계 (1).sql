...
- 날짜의 최댓값 = 가장 최근 날짜
- 행 수를 구할 때는 COUNT(*)를 사용함 (이때는 별을 쓰면 된다)
*/
...
-- 근무중인 직원의 급여 개수 조회
SELECT COUNT(salary) AS cnt_salary	-- NULL은 무시하고 COUNT
	FROM employee
	WHERE retire_date IS NULL;
    
-- 근무 중 직원 수
SELECT COUNT(*) AS emp_count  -- 테이블의 행의 개수 (열 이름이 바뀔 수 있으므로 *)
	FROM employee
	WHERE retire_date IS NULL;
...
SELECT AVG(salary) AS avg_salary
	FROM employee
	WHERE retire_date IS NULL; -- 6060.0000 (집계할 때, 고수애 직원은 빼고)

-- 급여 합을 직원 수로 나눠서 급여 평균 조회
SELECT SUM(salary) / COUNT(*) AS avg_salary
	FROM employee
	WHERE retire_date IS NULL; -- 5681.2500 (고수애 직원도 넣고)

-- 급여가 NULL이면 0으로 대체해서 급여 평균 조회
SELECT AVG(IFNULL(salary, 0)) AS avg_salary
	FROM employee
	WHERE retire_date IS NULL; -- 5681.2500


-- 3) 그룹별 집계
-- titanic.groupby(by='Pclass', as_index=False)[['Fare']].sum()
...
-- 참고: emp_name 해당 그룹의 첫번째 행 찍어줌 (MySQL 버전 8.2에서는 에러가 남)
...
SELECT dept_id, COUNT(*) AS emp_count 	-- 5)
	...
	GROUP BY dept_id					-- 3)
	HAVING COUNT(*) >= 3				-- 4) 집계함수를 그대로 한 번 더 쓸 수 밖에 없음.
	ORDER BY emp_count DESC; 			-- 6) 논리적인 sql 처리 순서로, 별치응을 쓸 수 있다.
...
SELECT dept_id, COUNT(*) AS emp_count
	...
	WHERE retire_date IS NULL
	GROUP BY dept_id
	HAVING emp_count >= 3	-- MySQL에서 쓸 수 있지만, 이도 최신버전에서는 안됨.
