...
-- 변수 값 출력
SET @Today = CURDATE();
SELECT @Today; -- 변수값에 의해서 실행 (변수는 해당 쿼리안에서만 존재한다고 생각하자)
...
-- 데이터베이스 연결
USE hrdb2019;  -- 왼쪽창에 DB를 더블클릭해줌

-- 현재 데이터베이스 확인
SELECT DATABASE();

-- 직원 정보 조회
SELECT * FROM employee;  -- 개발자는 별 볼 일 없다^^
...
-- 원하는 열을 원하는 순서로 나열 (물리적으로 저장된 열의 순서는 의미가 없다)
...
SELECT *
	FROM employee
	WHERE emp_id = 'S0001';
-- df.loc[df['emp_id'] == 'S0001'].head()    
-- 효율적인 데이터 처리는 SQL이 최선
...
SELECT *
	FROM employee
	WHERE dept_id = 'MKT' AND gender='M';
...
SELECT emp_name, emp_id, dept_id, gender, hire_date, phone
	FROM employee
	WHERE dept_id = 'SYS';  -- 오라클은 대소문자를 구분함
...
SELECT emp_name, emp_id, dept_id, gender, hire_date, phone
    ...
    -- 형태는 상관 없고 날짜만 유효하면 된다.
    -- '2015.03.01' '2015/03/01' '20150301' 
...
-- 참고: 인텍스 만들기 (책의 맨뒤에 있는 색인 만들기)
CREATE INDEX IDX ON employee(hire_date);
-- CREATE
-- ALTER
-- DROP
...
SELECT emp_name, emp_id, dept_id, hire_date, phone
    ...
    -- sql tuning 관점에서 아래 방법은 좋은 방법이 아님! (column에 함수를 취하지말라)
	-- WHERE YEAR(hire_date) = 2016;
    -- WHERE hire_date LIKE '2016%';
...
-- 판다스에서 between(A, B) / isin([A, B, C, D])
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
	WHERE hire_date BETWEEN '2016-01-01' AND '2016-12-31'; -- 경계값 포함
